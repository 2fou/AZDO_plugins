    ...
    useEffect(() => {
        const initializeComponent = async () => {
            try {
                ...
                const fieldValue = await workItemFormService.getFieldValue('Custom.AnswersField');
                ...
                if (fieldValue) {
                    try {
                        ...
                        const totalDeliveries = Object.values(answers).reduce((total, entry) => total + entry.entries.length, 0);
                        const completedDeliveries = Object.values(answers).reduce((total, entry) =>
